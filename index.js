...
app.get("/gets/:burc/:ozellik", async (req, res) => {
    ...
    await fetch(API_URI_3.replace('{0}', slugify(burc)).replace('{1}', slugify(ozellik)))
        ...
        .then(body => {
            ...
            let yorum;
            if (ozellik === "unluler") {
                yorum = $('body > div.page-wrapper.news-detail-page.Article.o-visible > section.news-detail-content > div.container > div:nth-child(5) > div.col-xl-17.col-lg-16.news-left-content > div.news-content.readingTime > ul').text();
            } else {
